...
def map(nc_data, var_name, co2_level, vmin, vmax):
    ...
    plt.title(f'Global Map: {var_name} mean over all time steps at {co2_level} ppm CO₂')
    ...
    for var in list(Data_Monthly['280'].variables.keys()):
        if var not in ['lat', 'lon', 'time', 'ta', 'ua', 'va', 'hus', 'wap', 'spd','lev']:
            ...
            for co2_level in ['280', '350', '450', '650', '850', '1150']:
                mean_data = np.ma.mean(Data_Daily[co2_level].variables[var][-36000:], axis=0)
            ...
            for co2_level in ['280', '350', '450', '650', '850', '1150']:
                map(Data_Daily[co2_level], var, co2_level, vmin, vmax)
